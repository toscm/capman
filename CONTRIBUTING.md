...
3. Use `capmake format` to format json files inside `macos/`
   - `capmake build` – compose the fragments into `macos/capman-v0.3.0.json`
   ...
   - `capmake install`
     - rebuild the bundle `macos/capman-v0.3.0.json`,
...
4. If you touch macOS rules, run `capmake format && capmake build` to refresh
   `macos/capman-v0.3.0.json`, then `capmake install` to update your local
